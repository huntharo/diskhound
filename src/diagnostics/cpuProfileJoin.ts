...
/**
 * Join consecutive CPU profile windows into one .cpuprofile.
 *
 * Ported from PwrAgnt's cpu-profile-history.ts. CDP returns a new
 * node-id namespace on every Profiler.stop, so this keeps one root,
 * remaps every reference, and keeps sample timestamps monotonic. The
 * stop/start gap between windows is charged to an "(unrecorded)" node,
 * not to the next JS function.
 */
  ...
  for (const profile of profiles) {
    ...
    if (previousEndTime !== undefined) {
      if (profile.startTime > previousEndTime) {
        ...
        // A sample describes the frame from its timestamp until the next
      ...
      // keeps the previous window's stack out of this window's first
      // sampling interval (and out of a GC sample with no JS stack).
