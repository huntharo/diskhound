...
describe("buildFolderTreeFromIndex scaling", () => {
  it("rolls up folders and trims each folder's file list in n log n", async () => {
    ...
    const run = async (count: number) => {
      ...
      // The last record is in the nested tree, the longer of the two paths.
    ...
    const small = await run(4_000);
    const large = await run(32_000);
    expectNearLinear("buildFolderTreeFromIndex", small, large, {
      maxTotal: 32_000 * (60 + 2 * parseOps),
    });
  });
});
