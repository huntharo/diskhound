...
export function sidecarFromDirectoryRoots(
  ...
  // Shortest first, so a folder's parents are settled before it is.
  const byLength = [...acc.artifacts.keys()].sort((a, b) => a.length - b.length);
  const survivors = new Set<string>();
  for (const path of byLength) {
    if (hasAncestorIn(path, survivors)) acc.artifacts.delete(path);
    else survivors.add(pathKey(path));
  }
